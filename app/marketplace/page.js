...
export default function MarketplacePage() {
  ...
  const { hasNFT, isLoading: nftLoading } = useNFT()
  const { 
    userBadges, 
    userREP,  
    canPurchaseBadge,
  ...
  // Check NFT ownership and redirect if doesn't have one
  useEffect(() => {
    if (!nftLoading && !hasNFT) {
      router.push('/nft-mint')
    }
  }, [hasNFT, nftLoading, router])
  ...
  // Get equipped badges from contract or backend API
  useEffect(() => {
    const fetchEquippedBadges = async () => {
      if (!walletAddress) return
      
      try {
        // This would be a contract call or API endpoint
  ...
  // Load user listings from backend API
  useEffect(() => {
  ...
  // Show loading while checking NFT or auth
  if (nftLoading || !isConnected) {
    return (
      <SidebarProvider>
        <AppSidebar />
        <SidebarInset>
  ...
  const handlePurchase = async (badgeId) => {
    ...
    try {
      ...
      // Refresh badge data from blockchain
      await refreshData()
  ...
  const handleListForSale = async () => {
    ...
    try {
      // This would be an API call to create listing
      const newListing = {
      ...
      console.error('Failed to list badge:', error)
      toast.error('Failed to list badge')
    }
  }

  const handleCancelListing = async (listingId) => {
    try {
      // This would be an API call to cancel listing
  ...
  const handleUpdatePrice = async (listingId, newPrice) => {
    try {
      // This would be an API call to update price
                      ...
                      <Tabs defaultValue="all" className="w-full">
                        ...
                        <div className="badge-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {filteredBadges.map((badge) => {
                            ...
                            const canPurchase = badge.isUserListing ? userREP >= badge.price : canPurchaseBadge(badge)
