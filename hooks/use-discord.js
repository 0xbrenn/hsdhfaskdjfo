...
export function DiscordProvider({ children }) {
  ...
  useEffect(() => {
    const checkExistingSession = () => {
      const savedUser = sessionStorage.getItem('iopn-discord-user')
      const savedExpiry = sessionStorage.getItem('iopn-discord-expiry')
      ...
      if (savedUser && savedExpiry) {
        ...
        if (expiry > Date.now()) {
          ...
          sessionStorage.removeItem('iopn-discord-user')
          sessionStorage.removeItem('iopn-discord-expiry')
        }
      }
  ...
  const processOAuthCallback = (token) => {
    try {
      ...
      // Set expiry (2 hours for sessionStorage)
      ...
      sessionStorage.setItem('iopn-discord-user', JSON.stringify(discordUserData))
      sessionStorage.setItem('iopn-discord-expiry', expiryTime.toString())
  ...
  const refreshToken = async () => {
    if (isDiscordConnected) {
      ...
      sessionStorage.setItem('iopn-discord-expiry', newExpiry.toString())
      return true
    }
    return false
  }
  ...
  // Skip Discord connection (no longer stored)
  const skipDiscord = () => {
    // Just a flag in memory, not persisted
    // This prevents the "discord-skipped" localStorage pollution
  }
